...
class CloudFormationCognitoUserPoolGroupIntegrationTest {
    ...
    void createStackProvisionsGroupsVisibleToCognito() {
        ...
        cognito("ListGroups", "{\"UserPoolId\": \"" + poolId + "\"}")
            ...
            .body("Groups.GroupName", containsInAnyOrder("admin", "student"));
    ...
    void removingAGroupFromTheTemplateDeletesOnlyThatGroup() {
        // A group is keyed by pool id plus group name and its physical id carries only the name,
        // so cleanup has to read the pool id back off the resource. Deleting the whole stack would
        // take the pool with it and hide a no-op delete behind a missing pool; removing one group
        // from the template leaves the pool and the sibling group as the discriminator.
        String suffix = Long.toString(System.nanoTime(), 36);
                  ...
                  "Resources": {
                    "UserPool": {
                      ...
                      "Properties": {
                        ...
                        "Schema": [
                          { "Name": "StudentNo", "AttributeDataType": "String", "Mutable": true },
